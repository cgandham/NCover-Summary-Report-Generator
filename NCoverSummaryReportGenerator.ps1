...
if (!(Test-Path "\\path\NCover\Latest"))           #create file if it does not exist
{
   New-Item -path "\\path\NCover\Latest" -type "directory"
}

if (!(Test-Path "\\path\NCover\Archive"))           #create file if it does not exist
{
   New-Item -path "\\path\NCover\Archive" -type "directory"
}

Move-Item -force \\path\NCover\Latest\*.html \\hyd-qatools\NCover\Archive
